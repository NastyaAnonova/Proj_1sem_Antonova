...
with sq.connect('tourist.db') as con:
    ...
    ('Токио-Шопинг', 'Япония', 'Токио', '2021-09-01', '2021-09-10', 12000),
    ('Париж-Осмотр достопримечательностей', 'Франция', 'Париж', '2021-10-01', '2021-10-05', 5500),
    ('Греция-Отдых на море', 'Греция', 'Санторини', '2021-11-01', '2021-11-07', 8000),
    ...
    ('Кейптаун-Сафари', 'Южная Африка', 'Кейптаун', '2022-04-01', '2022-04-10', 12500),
    ('Бангкок-Тематическая экскурсия по храмам', 'Таиланд', 'Бангкок', '2022-05-01', '2022-05-05', 5000),
    ('Египет-Отдых на курорте', 'Египет', 'Каир', '2022-06-01', '2022-06-07', 1200);""")
    else:
        pass
...
with sq.connect('tourist.db') as con:
    ...
    print(cur.execute(f"""SELECT * FROM reservations WHERE id_tour in (SELECT id FROM tours WHERE city='{input('Введите название города (Например, Токио): ')}')""").fetchall())
    #4 Вывести список всех туристов, сделавших бронирование в определенный период времени
    print(cur.execute(f"""SELECT * FROM tourists WHERE id in (SELECT id_tourist FROM reservations WHERE booking_date BETWEEN '{input('Введите начало периода (Например, 2022-10-10): ')}' AND '{input('Введите конец периода (Например, 2022-11-18): ')}')""").fetchall())
    ...
    print(cur.execute(f"""SELECT * FROM tourists WHERE id IN (SELECT id_tourist FROM reservations WHERE id_tour IN (SELECT id FROM tours WHERE lable='{input('Введите название тура (Например, Кейптаун-Сафари): ')}'))""").fetchall())
    #9  Вывести список всех туристов, которые сделали бронирование на тур в указанную дату
    print(cur.execute(f"""SELECT * FROM tourists WHERE id IN (SElECT id_tourist FROM reservations WHERE booking_date='{input('Введите дату бронирования тура (Например, 2022-10-10): ')}')""").fetchall())
...
with sq.connect('tourist.db') as con:
    ...
    # #17 Обновление названия тура на "Египет-отдых на курорте" для всех бронирований с id_тура равным 3
    # cur.execute(f"""UPDATE tours SET lable='Египет-отдых на курорте' WHERE id=3""")
...
with sq.connect('tourist.db') as con:
    ...
    # cur.execute(f"""DELETE FROM reservations WHERE booking_date='{input('Введите дату бронирования (Например, 2022-10-10): ')}'""")
    ...
    # cur.execute(f"""DELETE FROM reservations WHERE id_tourist IN (SELECT id FROM tourists WHERE mobile_number='{input('Введите номер телефона (Например, +393498765432): ')}')""")
    # #6 Удалить все бронирования, сделанные туристом с определенной электронной почтой
    # cur.execute(f"""DELETE FROM reservations WHERE id_tourist IN (SELECT id FROM tourists WHERE email='{input('Введите электронную почту (Например, mueller@gmail.com): ')}')""")
    # #7 Удалить все бронирования на туры, начинающиеся после определенной даты
    # cur.execute(f"""DELETE FROM reservations WHERE booking_date>'{input('Введите дату (Например, 2023-01-20): ')}'""")
    # #8 Удалить всех туристов, которые забронировали тур в определенную страну
    # cur.execute(f"""DELETE FROM tourists WHERE id IN (SELECT id_tourist FROM reservations WHERE id_tour IN (SELECT id FROM tours WHERE country='{input('Введите страну (Например, Россия): ')}'))""")
    # #9 Удалить все бронирования на туры, заканчивающиеся до определенной даты
    # cur.execute(f"""DELETE FROM reservations WHERE id_tour IN (SELECT id FROM tours WHERE end_date < '{input('Введите дату (Например, 2022-02-10): ')}')""")
    # #10  Удалить все бронирования, сделанные на тур с определенной ценой
    # cur.execute(f"""DELETE FROM reservations WHERE id_tour IN (SELECT id FROM tours WHERE price = '{input('Введите цену (Например, 5000): ')}')""")
